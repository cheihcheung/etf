...
/**
 * 带重试的请求封装（指数退避）
 * 将 axios 错误转换为普通 Error，避免不可序列化对象跨进程传递
 */
 ...
 *   - 51/56 开头(沪市ETF) → sh + 代码
 *   - 15/16 开头(深市ETF) → sz + 代码
 *   - 1B 开头(东财系指数) → sh + 补齐6位
 */
...
/**
 * 获取单只 ETF 的实时行情报价
 * 接口: https://qt.gtimg.cn/q=sh510300 (GBK 编码)
 */
